...
function generateReport(flags = {}) {
  ...
  if (flags.open) {
    // execFile + Arg-Array statt exec(string): outputPath kann aus --output stammen,
    // String-Interpolation in eine Shell waere ein Injection-Vektor (semgrep detect-child-process).
    const { execFile } = require('child_process');
    if (process.platform === 'win32') {
      execFile('cmd', ['/c', 'start', '', outputPath]);
    } else {
      execFile(process.platform === 'darwin' ? 'open' : 'xdg-open', [outputPath]);
    }
