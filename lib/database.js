...
function initDatabase(dbPath) {
  ...
  db.exec(`
    ...
    -- LLM-extracted metadata (category, summary, entities, language).
    -- Optional layer that depends on a configured LLM provider.
...
function upsertEnrichment(noteId, data) {
  stmt('upsertEnrichment', `
    INSERT OR REPLACE INTO enrichments
      (note_id, category, summary, entities, language, content_hash, enriched_at)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(
    noteId,
    data.category || null,
    data.summary || null,
    Array.isArray(data.entities) ? JSON.stringify(data.entities) : (data.entities || null),
    data.language || null,
    data.content_hash || '',
...
function getEnrichment(noteId) {
  const row = stmt('getEnrichment', `
    SELECT category, summary, entities, language, content_hash, enriched_at
    FROM enrichments WHERE note_id = ?
  `).get(noteId);
  if (!row) return null;
  return {
...
function getUnenrichedNotes(limit = 100) {
  return getDb().prepare(`
    SELECT n.note_id, n.title, n.folder, n.content_hash
    FROM notes n
    LEFT JOIN enrichments e ON e.note_id = n.note_id
...
function getEnrichmentStats() {
  ...
  const enriched = getDb().prepare('SELECT COUNT(*) as c FROM enrichments').get().c;
  const fresh = getDb().prepare(`
    SELECT COUNT(*) as c FROM enrichments e
    JOIN notes n ON n.note_id = e.note_id
    WHERE e.content_hash = n.content_hash
  `).get().c;
  const categories = getDb().prepare(`
    SELECT category, COUNT(*) as count FROM enrichments
    WHERE category IS NOT NULL
    GROUP BY category ORDER BY count DESC
  `).all();
  return { total, enriched, fresh, stale: enriched - fresh, categories };
}
