...
const { requireApiUrl } = require('../../config');

class OpenAILLM extends LLMAdapter {
  constructor(config) {
    super(config);
    // 🚨 No default endpoint (07.09.2026). This used to fall back to
    // `https://api.openai.com/v1`, which turned "endpoint not configured" into
    // "call OpenAI directly" — silently, past any gateway. Now it is an error.
    // Normalize URL: strip trailing /v1/, ensure no trailing slash
    const raw = requireApiUrl(config.url).replace(/\/+$/, '');
