...
function buildSnapshot() {
  ...
  // 1.1 features: gracefully degrade if tables don't yet exist
  let ftsCount = 0;
  let enrichmentStats = null;
  let embeddingStats = null;
  try { ftsCount = getFtsCount(); } catch { /* not migrated */ }
  try { enrichmentStats = getEnrichmentStats(); } catch { /* not migrated */ }
  try { embeddingStats = getEmbeddingStats(); } catch { /* not migrated */ }
  ...
  return {
    ...
    indexes: {
      fts_indexed_notes: ftsCount,
      enrichment: enrichmentStats
        ? { enriched: enrichmentStats.enriched, fresh: enrichmentStats.fresh, stale: enrichmentStats.stale }
        : null,
      embeddings: embeddingStats
        ? {
            embedded: embeddingStats.embedded,
            fresh: embeddingStats.fresh,
            stale: embeddingStats.stale,
            models: embeddingStats.models
          }
        : null
    },
    totals: {
