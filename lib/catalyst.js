...
// Send a chat completion via the configured LLM adapter.
// Backwards-compatible signature: llmRequest(config, messages, opts).
async function llmRequest(config, messages, opts = {}) {
  const llm = createLLM(config);
  return llm.chat({
    messages,
    temperature: opts.temperature ?? 0.8,
    maxTokens: opts.maxTokens ?? 1500,
    returnMeta: opts.returnMeta || false
  });
}
...
async function generateCatalysts(count = 3) {
  ...
  // Validate provider is available. The 'none' adapter throws a helpful
  // error when chat() is called, so we don't need to gate here -- but a
  // dry instantiation surfaces config errors early.
  const llm = createLLM(config);
  if (llm.name === 'none') {
    throw new Error(
